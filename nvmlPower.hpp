...
void nvmlAPIRun(const char * output_file);
void nvmlAPIEnd();
void *powerPollingFunc(void * output_file);
...
void *powerPollingFunc(void * output_file)
{
	...
	while (pollThreadStatus)
	{
		...
		fprintf(fp, "%lld %lf W\n", std::time(nullptr), (powerLevel)/1000.0);
		mean_power += (powerLevel)/1000.0;
		nb_iter++;
...
void nvmlAPIRun(const char * output_file)
{
	...
	int iret = pthread_create(&powerPollThread, NULL, powerPollingFunc, (void*) output_file);
	if (iret)
	{
