...
# run test with slurm
#
def run_sample_with_slurm(P0,P1,P2,x,y,z,yBlockDim,xBlockDim, e_out_cublas, e_out_tee_acm, partition="volta-x86", get_metrics=True, with_slurm=True, run_kernels_multiple_times=False, cuda_exe_path=""):
...
#
def run_multiple_times_and_get_expected_metrics(P0,P1,P2,x,y,z,yBlockDim,xBlockDim, partition, with_slurm, kernel_names, kernels_info, path_to_executable,e_out_cublas, e_out_tee_acm, iter=10, fused_kernel=False,P3=0):
    ...
    for _ in range(iter):
        ...
        if fused_kernel:
            ...
            exp_metrics = run_sample_with_slurm(P0,P1,P2,x,y,z,yBlockDim,xBlockDim, e_out_cublas+csv_file, e_out_tee_acm+csv_file, partition=partition, get_metrics=True, with_slurm=with_slurm, run_kernels_multiple_times=True,cuda_exe_path=path_to_executable)
        if DEBUG:
...
#
def run_kernels_and_get_info(kernel_names,P0,P1,P2,x,y,z,yBlockDim,xBlockDim,e_out_cublas, e_out_tee_acm, partition="volta-x86", with_slurm=True, path_to_executable=""):

    # get --print-gpu-trace metrics
    general_info = run_sample_with_slurm(P0,P1,P2,x,y,z,yBlockDim,xBlockDim,"tmp1.csv","tmp2.csv", partition=partition, get_metrics=False, with_slurm=with_slurm,cuda_exe_path=path_to_executable)
    if DEBUG:
    ...
    nvprof_metrics = run_sample_with_slurm(P0,P1,P2,x,y,z,yBlockDim,xBlockDim,"tmp1.csv","tmp2.csv", partition=partition, get_metrics=True, with_slurm=with_slurm,cuda_exe_path=path_to_executable)
    if DEBUG:
    ...
    run_multiple_times_and_get_expected_metrics(P0,P1,P2,x,y,z,yBlockDim,xBlockDim, partition, with_slurm, kernel_names, kernels_info, path_to_executable, e_out_cublas, e_out_tee_acm, iter=10)
...
def run_kernels_and_profile(kernel_names,P0,P1,P2,x,y,z,yBlockDim,xBlockDim,e_out_cublas, e_out_tee_acm, partition="volta-x86", with_slurm=True, path_to_executable=""):
...
#
# get metrics printed by the program for a single run
#
def run_single_and_get_expected_metrics(tmp_info,P0,P1,P2,x,y,z,yBlockDim,xBlockDim, partition, with_slurm, kernel_names, path_to_executable,e_out_cublas, e_out_tee_acm, run_number, fused_kernel=False, P3=0):
    ...
    if not os.path.exists(e_out_tee_acm):
        os.makedirs(e_out_tee_acm)        
    # run iter number of itertions
    csv_file = "/run_{}.csv".format(run_number)
    if fused_kernel:
        exp_metrics = fused_run_sample_with_slurm(P0,P1,P2,P3,x,y,z,yBlockDim,xBlockDim, e_out_cublas+csv_file, e_out_tee_acm+csv_file, partition=partition, get_metrics=True, with_slurm=with_slurm, run_kernels_multiple_times=True,cuda_exe_path=path_to_executable)
    else :
        exp_metrics = run_sample_with_slurm(P0,P1,P2,x,y,z,yBlockDim,xBlockDim, e_out_cublas+csv_file, e_out_tee_acm+csv_file, partition=partition, get_metrics=True, with_slurm=with_slurm, run_kernels_multiple_times=True,cuda_exe_path=path_to_executable)
